...
def deploy():
    ...
    # 가상환경 업데이트
    _update_virtualenv()
    # 서버가 가동될 때 홈디렉토리, 로그파일의 위치 등등 설정
    # *.conf가 수정 및 생성된다
    # wsgi.py로 엔트리포인트가 설정되어 있다
    _make_virtualhost()
...
def _get_latest_apt():
    ...
    if update_or_not == 'y':
        ...
        # -y     => 물어보면 무조건 yes라는 의미
        # sudo() => root 권한으로 실행해라
        # 즉, 제일 먼저 할 일은? => $> sudo apt-get update && apt-get -y upgrade
